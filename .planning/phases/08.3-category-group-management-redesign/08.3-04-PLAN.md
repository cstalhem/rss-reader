...
<task type="auto">
  ...
  **Wire Move to Group dialog in `CategoriesSection.tsx`:**
  ...
  - Render `<MoveToGroupDialog>` with:
    ...
    - `onCreate={(groupName) => { createCategory(groupName); /* After creation, the new parent won't have an ID yet. Use a pattern: create category, then on the categories query refetch, find the new category by name and batch-move. OR: simplify by just creating the group and closing — user moves in a second step. Per simplicity-first: create the group AND move selected items. Use createCategoryMutation.mutateAsync to get the new category, then call batchMove with its ID. */ }}`.
  - For `onCreate` flow: use `apiCreateCategory` directly (imported from api.ts) with `await`, get the returned category, then call `batchMove` with the new category's ID. Wrap in a handler function. Since `useCategories.createCategory` doesn't return the created object, either:
    a. Import `apiCreateCategory` from `@/lib/api` directly and use it in the handler, then invalidate manually, OR
    b. Update `createCategoryMutation` in `useCategories` to use `mutateAsync` and return the result.
  - Prefer option (a) for simplicity — keep the handler in CategoriesSection. After the `apiCreateCategory` + `batchMoveCategories` calls, invalidate `["categories"]` via `queryClient` (import `useQueryClient`). OR even simpler: the `batchMove` mutation already invalidates. So just call `apiCreateCategory`, then `batchMove` with the returned id. The batchMove mutation will invalidate everything.
  </action>
  <verify>
