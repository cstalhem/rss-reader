...
must_haves:
  truths:
    - "Hide endpoint sets is_hidden=True without modifying weight"
    - "Unhide endpoint clears parent_id (returns to ungrouped) and only resets weight if it was block"
...
<task type="auto">
  ...
  **Fix unhide endpoint** (`PATCH /api/categories/{id}/unhide`):
  - Add `category.parent_id = None` to clear the parent reference. Per CONTEXT.md, unhide returns category to ungrouped.
  ...
  **Add batch hide endpoint** (`POST /api/categories/batch-hide`):
  ...
  - For each category_id: set `is_hidden = True`. Do NOT modify weight.
  - Return `{ ok: true, updated: N }`.
