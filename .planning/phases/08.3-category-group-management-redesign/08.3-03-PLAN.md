...
<task type="auto">
  ...
  **Create `CategoryContextMenu.tsx`:**
  ...
  - Inside `Portal > Menu.Positioner > Menu.Content`:
    ...
    - Always: `Menu.Item` for "Hide" (calls `onHide`). Add a helper text or tooltip: "Removes from group and hides from LLM" — this communicates that hiding also clears the parent group assignment.
    - `Menu.Separator`.
  ...
  **Update `CategoryChildRow.tsx`:**
  - Replace the hover-reveal pencil/trash/hide buttons with `<CategoryContextMenu type="child" isWeightOverridden={isOverridden} onResetWeight={onResetWeight} onRename={() => setIsRenaming(true)} onHide={onHide} onDelete={onDelete} />`.
  ...
  **Update `CategoryUngroupedRow.tsx`:**
  - Replace the hover-reveal pencil/trash/hide buttons with `<CategoryContextMenu type="ungrouped" onRename={() => setIsRenaming(true)} onHide={onHide} onDelete={onDelete} />`.
  - No `isWeightOverridden` or `onResetWeight` props needed — ungrouped rows have no parent to reset to.
  - Remove the hover-reveal `Flex` containing the `IconButton` components.
  - Keep `isHovered` state only if still needed for the "New" badge X icon expand.
