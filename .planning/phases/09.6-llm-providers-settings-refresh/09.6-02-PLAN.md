...
must_haves:
  truths:
    ...
    - "Disconnect uses ConfirmDialog that communicates model assignments will be cleared"
...
- `handleDisconnect(providerId)`: Call `disconnectMutation.mutate(providerId)`, collapse panel on success
...
- Disconnect uses `ConfirmDialog` (from `@/components/ui/confirm-dialog`):
  - Confirm message: "Disconnecting will also remove any model assignments using Ollama from scoring and categorization."
  - Confirm action label: "Disconnect"
  - On confirm: calls `onDisconnect()`
...
6. Click Disconnect -- verify ConfirmDialog warns about model assignments. Confirm, verify pill disappears.
    </manual>
  </verify>
...
9. Disconnect ConfirmDialog communicates model assignment clearing
...
- Disconnect clearly communicates model assignment impact to user
