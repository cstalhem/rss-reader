...
must_haves:
  truths:
    ...
    - "GET /api/models returns available models aggregated from all enabled providers, each tagged with provider name"
  ...
  artifacts:
    - path: "backend/src/backend/routers/ollama.py"
      provides: "Provider endpoints (list, disconnect, config), task-route endpoints, and aggregated models endpoint (with extraction TODO)"
...
<success_criteria>
- Backend serves provider endpoints (list, disconnect, config), task-route endpoints, and aggregated models endpoint
