...
### Pitfall 3: useListCollection Reactivity
**What goes wrong:** `useListCollection` takes `initialItems` -- if the items array changes (models installed/deleted), the collection may not update.
**Why it happens:** The `initialItems` prop is used as the initial value, not a reactive binding.
**How to avoid:** Verify whether `useListCollection` re-initializes when `initialItems` reference changes. If not, use `collection.setItems()` or recreate the hook instance via `key` prop on the parent.
...
1. **`useListCollection` reactivity with changing `initialItems`**
   - What we know: Chakra docs show `useListCollection({ initialItems })` pattern. The name "initial" suggests it might not re-sync on prop changes.
   - What's unclear: Whether the hook re-initializes when the `initialItems` array reference changes (e.g., after downloading a new model).
   - Recommendation: Test during implementation. If not reactive, use `key` prop on the Combobox parent (keyed on `models.length` or similar) to force remount, or call `collection.setItems()` in a `useEffect`.
...
- `useListCollection` reactivity behavior — inferred from "initial" naming convention, needs validation during implementation
...
- Combobox grouping: MEDIUM — verified API exists via MCP, but `useListCollection` reactivity with dynamic data needs testing
