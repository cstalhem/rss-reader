...
See `09.6-CONTEXT.md` for all locked decisions, Claude's discretion items, and deferred ideas. CONTEXT.md is the single source of truth for design decisions.
</user_constraints>
...
├── OllamaHealthBadge.tsx        # DELETE -- health shown inline in OllamaProviderPanel
...
- Pills show brand icon + name only -- no health indicator on pills (health is shown inside the expanded provider panel)
...
**What:** When no providers are configured, show EmptyState with "Add Provider" button. Adding Ollama is a frontend-only state change (pending provider) until the user saves.
**When to use:** First-time setup, or after disconnecting all providers (disconnect deletes the row, so re-add always starts fresh).
**Backend flow:**
1. User clicks "Add Provider" -> selects Ollama in dialog -> provider added as "pending" (frontend-only)
2. Panel opens in setup mode with defaults. User clicks Save.
3. Frontend calls `PUT /api/providers/ollama/config` with config (this triggers `upsert_ollama_provider_config` which creates the `LLMProviderConfig` row)
4. Provider appears as a saved pill, settings panel transitions to edit mode
...
- **Don't show health on pills** — Pills show brand icon + name only. Health status is shown inside the expanded provider panel.
...
**How to avoid:** When adding Ollama, immediately expand its settings panel so the user can verify/change connection settings. The save can happen with default values -- the health status inside the expanded panel provides feedback.
...
**How to avoid:** Backend disconnect endpoint deletes the `LLMProviderConfig` row AND deletes `LLMTaskRoute` rows referencing that provider in a single transaction. Re-adding a provider always starts fresh (no preserved config).
...
- `ollama_categorization_model` / `ollama_scoring_model` / `ollama_thinking` / `active_llm_provider` columns in UserPreferences -- removed in this phase (superseded by LLMProviderConfig + LLMTaskRoute)
- `ollama_use_separate_models` renamed to `use_separate_models` in UserPreferences -- this is a task routing preference, not provider-specific
...
2. **~~Disconnect semantics: delete row vs disable~~** (RESOLVED)
   - Decision: **Delete the row.** Disconnect deletes the `LLMProviderConfig` row and all `LLMTaskRoute` rows referencing that provider in a single transaction. Re-adding a provider always starts fresh through the Add Provider flow. This is the simplest approach and avoids managing disabled/re-enable state transitions.
...
4. **~~Backend: "list configured providers" endpoint~~** (RESOLVED)
   - Decision: Add `GET /api/providers` endpoint that returns configured (existing) providers from the `LLMProviderConfig` table. Since disconnect deletes the row, all returned providers are active. The `enabled` column becomes unused but is left in the schema for now.
