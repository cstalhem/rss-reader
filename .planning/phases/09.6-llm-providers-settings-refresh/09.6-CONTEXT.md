...
- **Health status**: shown inside the expanded Ollama provider panel (not on the pill). Simplified from original design -- pill shows brand icon only, health visible when panel is open
...
- Provider pills show **brand icon** only -- no health indicator on pills
