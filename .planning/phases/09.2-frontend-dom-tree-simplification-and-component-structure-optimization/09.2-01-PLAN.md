...
must_haves:
  truths:
    ...
    - "Mobile shows a NativeSelect dropdown at top of content area for section switching"
...
<task type="auto">
  ...
  <action>
    ...
    1. **`app/settings/layout.tsx`** ("use client" -- needs `usePathname` and `useRouter` for nav):
       - Import Header, SettingsSidebar (updated in Task 2), and necessary Chakra components.
       ...
       - Mobile dropdown nav inside content area: `Box display={{ base: "block", md: "none" }} mb={4}` containing a `SettingsMobileNav` component (inline or co-located).
       - Mobile dropdown: `NativeSelect.Root size="sm"` with `NativeSelect.Field` whose value comes from the last segment of `usePathname()` and `onChange` calls `router.push(\`/settings/${e.target.value}\`)`. Options: feeds, interests, categories, ollama, feedback.
