...
- truth: "Parent delete action splits the group (parent becomes ungrouped, children released) rather than deleting the parent category"
  ...
  severity: blocker
  test: 10
  root_cause: "Backend DELETE endpoint removes parent from both children_map AND manually_created. Parent disappears entirely if it was manually created and never assigned by LLM. No separate 'split group' action exists — only delete."
  artifacts:
  ...
  missing:
    - "Keep parent in manually_created when splitting group"
    - "Add separate split-group API action or frontend-only ungroup logic"
    - "Add visible split icon distinct from delete on parent rows"
...
- truth: "Drag-and-drop is smooth, responsive, and visually correct"
  ...
  missing:
    - "Add closestCenter or custom collision detection strategy"
    - "Separate sortable and droppable concerns (don't use both on same element)"
    - "Fix z-index and pointer-events on DragOverlay"
    - "Consider deeper @dnd-kit/core optimization before library switch"
