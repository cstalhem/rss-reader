...
must_haves:
  truths:
    - "GET /api/providers returns configured providers (only rows that exist -- disconnect deletes the row)"
    - "DELETE /api/providers/{provider} deletes the LLMProviderConfig row and all LLMTaskRoute rows referencing that provider in a single transaction"
    ...
    - "GET /api/models returns available models aggregated from all configured providers, each tagged with provider name"
    - "GET /api/task-routes returns current model assignments (task, provider, model) and use_separate_models (read from UserPreferences)"
    - "PUT /api/task-routes saves model assignments independently from provider config; saves use_separate_models to UserPreferences"
    - "UserPreferences.ollama_use_separate_models renamed to use_separate_models via schema migration"
    - "Legacy UserPreferences columns removed: ollama_categorization_model, ollama_scoring_model, ollama_thinking, active_llm_provider"
    - "Legacy fallback _build_legacy_ollama_config removed from deps.py"
    ...
    - "Frontend useAvailableModels hook fetches aggregated models from GET /api/models -- provider-agnostic"
  ...
  artifacts:
    - path: "backend/src/backend/routers/ollama.py"
      provides: "Provider endpoints (list, disconnect via DELETE, config), task-route endpoints, and aggregated models endpoint (with extraction TODO)"
    - path: "backend/src/backend/models.py"
      provides: "UserPreferences with use_separate_models (renamed from ollama_use_separate_models), legacy columns removed"
    - path: "backend/src/backend/deps.py"
      provides: "Legacy _build_legacy_ollama_config fallback removed, references to old UserPreferences columns removed"
    ...
    - path: "frontend/src/hooks/useModelAssignments.ts"
      provides: "useModelAssignments hook — reads/writes task routes, triggers rescore"
    - path: "frontend/src/hooks/useAvailableModels.ts"
      provides: "useAvailableModels hook — fetches aggregated models from all enabled providers"
  ...
  key_links:
    ...
    - from: "frontend/src/hooks/useModelAssignments.ts"
      ...
      pattern: "task.routes"
    - from: "frontend/src/hooks/useAvailableModels.ts"
      to: "/api/models"
      via: "fetchAvailableModels API function"
      pattern: "api/models"
...
1. **GET /api/providers** -- Query `LLMProviderConfig` table, return all rows. Since disconnect deletes the row, all returned providers are active/configured. If no rows exist, return an empty list.

2. **DELETE /api/providers/{provider}** -- Find the `LLMProviderConfig` row for the given provider name. Delete all `LLMTaskRoute` rows referencing this provider, then delete the `LLMProviderConfig` row itself. Both operations in a single transaction. Return `{ok: true}`. If provider not found, return 404.

Disconnect always deletes the row -- re-adding a provider starts fresh through the Add Provider flow. This is the simplest approach and avoids managing disabled/re-enable state transitions.
...
3. **PUT /api/providers/{provider}/config** -- Provider-agnostic URL that replaces the old `PUT /api/ollama/config`. Dispatch based on provider name:
   - For `"ollama"`: same logic as the existing `PUT /api/ollama/config` (validate OllamaProviderConfig, upsert LLMProviderConfig row, sync task routes via `sync_ollama_task_routes`).
   - For unknown providers: return 404.
   - Delete the old `PUT /api/ollama/config` endpoint -- the frontend is updated to use the new URL in this same plan (Task 2, step 10), so no backward compatibility alias is needed.

This endpoint handles both initial creation (user adds provider and saves for the first time) and subsequent updates (user edits host/port). The upsert in `upsert_ollama_provider_config` already handles both cases.

Also remove `preferences.active_llm_provider = "ollama"` from the save logic -- the `active_llm_provider` column is being deleted (see UserPreferences cleanup below).

**Task-route endpoints (model assignments):**

4. **GET /api/task-routes** -- Query `LLMTaskRoute` table. Return list of `{task: string, provider: string, model: string | null}`. The frontend needs this to populate the model selector independently from any provider's config. Also include `use_separate_models: bool` -- read from `UserPreferences.use_separate_models` (provider-agnostic, persists across provider disconnect/reconnect). If no UserPreferences row exists, default to `false`.

5. **PUT /api/task-routes** -- Accept `{categorization: {provider: string, model: string}, scoring: {provider: string, model: string}, use_separate_models: bool}`. Upsert the two `LLMTaskRoute` rows using existing `upsert_task_route()`. Save `use_separate_models` to `UserPreferences.use_separate_models`. Return `{ok: true}`.

**Aggregated models endpoint:**

6. **GET /api/models** -- Provider-agnostic endpoint that aggregates available models from all configured providers. Query `LLMProviderConfig` for all provider rows (all are active since disconnect deletes), resolve each via `get_provider()` from `llm_providers/registry.py`, call `list_models()` on each, and merge the results. Each model in the response is tagged with its provider name. Return `list[AvailableModel]`. If a provider's `list_models()` fails (e.g., Ollama is down), skip that provider and return models from the rest (or an empty list if all fail) -- do not let one provider's failure block the entire response. Per-provider model endpoints (e.g., `/api/ollama/models`) stay for provider-specific management (download, delete) inside provider panels.

**UserPreferences cleanup:**

7. **Schema migration** -- Add a version-gated migration in `database.py`:
   - Rename column: `ollama_use_separate_models` → `use_separate_models`
   - Drop columns: `ollama_categorization_model`, `ollama_scoring_model`, `ollama_thinking`, `active_llm_provider`
   - These columns are fully superseded by `LLMProviderConfig` + `LLMTaskRoute` tables and have been legacy-only since Phase 09.5.

8. **Update `models.py`** -- Remove the deleted columns from the `UserPreferences` model. Rename the field:
   ```python
   class UserPreferences(SQLModel, table=True):
       ...
       feed_refresh_interval: int = Field(default=1800)
   ```

9. **Update `deps.py`** -- Remove `_build_legacy_ollama_config` fallback function and all references to the deleted `UserPreferences` columns (`ollama_categorization_model`, `ollama_scoring_model`, `ollama_use_separate_models`). The `get_ollama_provider_config` function should return the config from `LLMProviderConfig` only -- if no row exists, it means the provider isn't configured (no legacy fallback needed).

**Response schemas to add in `schemas.py`:**
- `ProviderListItem(provider: str)`
- `AvailableModel(provider: str, name: str, size: int | None, parameter_size: str | None, quantization_level: str | None, is_loaded: bool | None)` -- superset of per-provider model fields; nullable fields accommodate providers that don't report them
...
- `TaskRouteAssignment(provider: str, model: str)`
  ...
  <verify>
    <automated>cd /Users/cstalhem/projects/worktrees/rss-reader/great-lehmann && uv run python -c "from backend.routers.ollama import router; routes = [r.path for r in router.routes]; print(routes); assert any('providers' in r for r in routes); assert any('task-routes' in r for r in routes); assert any(r == '/api/models' for r in routes); print('OK: all endpoints registered')"</automated>
    <manual>Check that GET /api/providers returns [] on cold start, PUT /api/providers/ollama/config creates a row, GET /api/task-routes returns current routes, PUT /api/task-routes updates model assignments, GET /api/models returns models tagged with provider name</manual>
  </verify>
  <done>Provider endpoints (list, disconnect via DELETE, config), task-route endpoints (get, save with use_separate_models from UserPreferences) all working. Provider config uses provider-agnostic URL pattern. Legacy UserPreferences columns removed, column renamed. Response schemas defined. TODO comments document extraction requirement.</done>
</task>
...
- `disconnectProvider(provider: string): Promise<{ok: boolean}>` -- DELETE /api/providers/{provider}
...
- `fetchAvailableModels(): Promise<AvailableModel[]>` -- GET /api/models
...
- `useMutation` for disconnect (DELETE) with `meta: { errorTitle: "Failed to disconnect provider" }`, invalidates `queryKeys.providers.all` and `queryKeys.taskRoutes.all` on settled (disconnect deletes provider row and task routes)
...
This hook is one of the model selector's two data dependencies (the other is `useAvailableModels`). It knows nothing about Ollama config, host/port, or any provider-specific state.

**9. useAvailableModels hook** -- `hooks/useAvailableModels.ts`:
- `useQuery` with `queryKeys.models.available` and `fetchAvailableModels` as queryFn
- `staleTime: 30_000`, `refetchOnWindowFocus: true` (same polling cadence as the old `useOllamaModels`)
- Returns `{ models, isLoading }` where `models` is `AvailableModel[]`
- This hook is provider-agnostic -- the backend handles aggregation. The model selector uses this instead of `useOllamaModels`.

**10. Update useOllamaConfig** -- Update `saveOllamaConfig` in `api.ts` to call `PUT /api/providers/ollama/config` instead of `PUT /api/ollama/config`. The hook itself stays the same -- it still manages Ollama-specific config (host/port). It just uses the new URL.
  </action>
  <verify>
    <automated>cd /Users/cstalhem/projects/worktrees/rss-reader/great-lehmann/frontend && bun run build 2>&1 | tail -20</automated>
    <manual>Verify providers/ directory structure exists with 5 provider modules, registry exports PROVIDER_REGISTRY with 5 entries, all three hooks compile (useProviders, useModelAssignments, useAvailableModels)</manual>
  </verify>
  <done>Provider plugin module structure created mirroring backend pattern. ProviderPlugin interface with isNew/onCancelSetup support, 5 provider modules with Logo placeholders, PROVIDER_REGISTRY, data types, API functions (using new provider-agnostic URLs), useProviders hook, useModelAssignments hook, useAvailableModels hook, and query keys all compile without errors.</done>
</task>

</tasks>

<verification>
1. Backend: `uv run ruff check backend/src/backend/routers/ollama.py backend/src/backend/schemas.py backend/src/backend/models.py backend/src/backend/deps.py` passes
2. Frontend: `bun run build` succeeds with no type errors
3. Query keys: `queryKeys.providers.all`, `queryKeys.models.available`, and `queryKeys.taskRoutes.all` exist
...
9. Old PUT /api/ollama/config endpoint removed
10. DELETE /api/providers/{provider} deletes both LLMProviderConfig and LLMTaskRoute rows
11. UserPreferences model has `use_separate_models` (not `ollama_use_separate_models`)
12. No references to deleted columns (ollama_categorization_model, ollama_scoring_model, ollama_thinking, active_llm_provider) remain
13. GET /api/task-routes reads use_separate_models from UserPreferences
</verification>

<success_criteria>
- Backend serves provider endpoints (list, disconnect via DELETE, config), task-route endpoints, and aggregated models endpoint
- Provider config uses provider-agnostic URL: PUT /api/providers/{provider}/config
- Disconnect (DELETE /api/providers/{provider}) deletes the row and task routes -- re-add always starts fresh
- use_separate_models lives in UserPreferences (provider-agnostic), accessed via task-route endpoints
- Legacy UserPreferences columns removed (ollama_categorization_model, ollama_scoring_model, ollama_thinking, active_llm_provider)
- Legacy _build_legacy_ollama_config fallback removed
...
- useAvailableModels fetches aggregated models from GET /api/models -- provider-agnostic
