...
### Theme D: Settings Page Header Standardisation

| ID | Recommendation | Files Affected | What Changes |
|----|----------------|---------------|--------------|
| D1 | Extract `SettingsPageHeader` component | All section components | Standardised header layout with title (fontSize="xl") + optional children slot for right-side actions + optional title badge slot |
| D2 | Fix inconsistent page title sizes | FeedsSection, GeneralSection, FeedbackPlaceholder | Currently use `fontSize="lg"` for page titles; should be `"xl"` (handled automatically by D1) |

Current header patterns vary across sections:

| Section | Title | Right-side element |
|---------|-------|--------------------|
| General | "General" | *nothing* |
| Feeds | "Feeds" | `<Button>Add Feed</Button>` |
| Interests | "Interest Preferences" | *nothing* |
| Categories | "Topic Categories" + `<Badge>N new</Badge>` | `<CreateCategoryPopover>` |
| Ollama | "Ollama" | `<OllamaHealthBadge>` |
| Feedback | "Feedback" | *nothing* |

`SettingsPageHeader` is a client component (CTAs need client state) that standardises the `Flex` + title styling and takes right-side content as children. The `page.tsx` files remain as thin Server Component pass-throughs -- the header cannot move there because of the client state dependency. The page files retain architectural value as the server/client boundary and future home for `metadata` exports.

**Expected outcome:** One component defines the settings header pattern. New settings pages reach for the component instead of copy-pasting. Title sizing is enforced structurally.

### Theme E: Settings Mobile Nav Overhaul

| ID | Recommendation | Files Affected | What Changes |
|----|----------------|---------------|--------------|
| E1 | Share navigation items between sidebar and mobile nav | SettingsSidebar.tsx, SettingsMobileNav.tsx, lib/constants.ts | Export shared `SETTINGS_SECTIONS` array (id, label, icon, href) used by both components. Eliminates duplicate arrays that drift apart. |
| E2 | Replace `NativeSelect` with Chakra UI `Select` | SettingsMobileNav.tsx | Custom dropdown with icons matching the desktop sidebar, instead of OS-native picker |
| E3 | Add missing "General" item | SettingsMobileNav.tsx | Resolved automatically by E1 -- shared array includes all 6 sections |

**Expected outcome:** Mobile nav matches desktop sidebar (same items, same icons), powered by a single shared data source. Visual consistency between mobile and desktop navigation.
